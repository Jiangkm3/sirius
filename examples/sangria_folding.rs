        ...
        layouter
            .assign_region(
                ...
                |mut region| {
                    ...
                    let z0_next_cell =
                        region.assign_advice(|| "z0_next", config.c, offset, || val_z0_next)?;
                    ...
                    let z1_next_cell =
                        region.assign_advice(|| "z1_next", config.c, offset, || val_z1_next)?;
...
pub fn setup_keys<const COMMITMENT_KEY_SIZE: usize, C1: CurveAffine, C2: CurveAffine>(
    ...
    let primary_key = unsafe {
        CommitmentKey::<C1>::load_or_setup_cache(cache_path, "sangria_bn256", COMMITMENT_KEY_SIZE)?
    };
...
fn main() {
    tracing_subscriber::fmt()
        .with_env_filter("info,sirius=warn")
        .init();

    let sc1_template = MyStepCircuit {
        private_inputs: [C1Scalar::from(0); 2],
    };
    let sc2 = trivial::Circuit::<1, C2Scalar>::default();

    let (pk, sk) = setup_keys::<COMMITMENT_KEY_SIZE, C1Affine, C2Affine>().unwrap();
